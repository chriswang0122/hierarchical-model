...
latent_dim = 16
dropout_rate = 0.5
model = HRNNVAE(vocab_size=datasets['train'].vocab_size,
...
for ep in range(epoch):
    ...
    for split in splits:
        ...
        totals = {'ELBO': 0., 'NLL': 0., 'KL': 0.,
                  'KL_zg': 0., 'KL_zl': 0., 'words': 0}
        ...
        for itr, (enc_inputs, dec_inputs, targets, lengths) in enumerate(dataloader):
            ...
            # forward
            logp, mu_zg, logvar_zg, mu_zl, logvar_zl = model(enc_inputs,
                                                             dec_inputs,
                                                             lengths)

            # calculate loss
            NLL_loss = NLL(logp, targets, lengths + 1)
            KL_zg_loss = KL_div(mu_zg, logvar_zg)
            KL_zl_loss = KL_div(mu_zl, logvar_zl)
            KL_loss = KL_zg_loss + KL_zl_loss
            ...
            totals['KL_zg'] += KL_zg_loss.item()
            totals['KL_zl'] += KL_zl_loss.item()
            ...
            if split == 'train':
                ...
                tracker['KL_zg'].append(KL_zg_loss.item() / bsize)
                tracker['KL_zl'].append(KL_zl_loss.item() / bsize)
                tracker['KL_weight'].append(KL_weight)

                # print statistics
                if itr % print_every == 0 or itr + 1 == len(dataloader):
                    print("%s Batch %04d/%04d, ELBO-Loss %.4f, "
                          "NLL-Loss %.4f, KL-Loss %.4f, KL-zg-Loss %.4f, "
                          "KL-zl-Loss %.4f, KL-Weight %.4f"
                          % (split.upper(), itr, len(dataloader),
                             tracker['ELBO'][-1], tracker['NLL'][-1],
                             tracker['KL'][-1], tracker['KL_zg'][-1],
                             tracker['KL_zl'][-1], tracker['KL_weight'][-1]))

        samples = len(datasets[split])
        print("%s Epoch %02d/%02d, ELBO %.4f, NLL %.4f, KL %.4f, "
              "KL-zg %.4f, KL-zl %.4f, PPL %.4f"
              % (split.upper(), ep, epoch, totals['ELBO'] / samples,
                 totals['NLL'] / samples, totals['KL'] / samples,
                 totals['KL_zg'] / samples, totals['KL_zl'] / samples,
...
ax2 = ax1.twinx()
lns2 = ax2.plot(tracker['KL_zg'], 'r', label='KL zg term')
lns3 = ax2.plot(tracker['KL_zl'], 'C1', label='KL zl term')
